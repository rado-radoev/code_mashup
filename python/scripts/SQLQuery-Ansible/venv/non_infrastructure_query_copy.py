...
class Query_Inventory(object):
    ...
    def get_data_from_db(self):
        ...
        cnxn = pyodbc.connect('Driver={SQL Server};Server=PRSMCTXSDK1\SQLEXPRESS;Database=ansibledata;Trusted_Connection=yes;')

        # Connect to SQL Express from Linux
        # cnxn = pyodbc.connect('DSN=ansibledata')

        cursor = cnxn.cursor()
        cursor.execute('SELECT * FROM ansibledata.dbo.AnsibleNonInfrastructure')
        ...
        for row in cursor:
            ...
            self.servers.append(Server(hostname=hostname, build_desc=build_desc, site=site, td_type=td_type))
